  ...
  for (int j = 1; j < n + 1; j++) {
    for (int i = 0; i < d[j].size(); i++) {
      ...
      for (int f = 0; f < m; f++) {
        ...
        if (max(gr[N[f]][j].honest, gr[N[f]][d[j][i]].honest) == -1)
          key1 = 1;
        else if (gr[N[f]][j].honest == -1)
          Min1 = gr[N[f]][d[j][i]].honest;
        else if (gr[N[f]][d[j][i]].honest == -1)
          Min1 = gr[N[f]][j].honest;
        else
          Min1 = min(gr[N[f]][j].honest, gr[N[f]][d[j][i]].honest);
        
        if (max(gr[N[f]][j].unhonest, gr[N[f]][d[j][i]].unhonest) == -1)
          key2 = 1;
        else if (gr[N[f]][j].unhonest == -1)
          Min2 = gr[N[f]][d[j][i]].unhonest;
        else if (gr[N[f]][d[j][i]].unhonest == -1)
          Min2 = gr[N[f]][j].unhonest;
        else 
          Min2 = min(gr[N[f]][j].unhonest, gr[N[f]][d[j][i]].unhonest);
